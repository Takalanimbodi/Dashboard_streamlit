...
import psycopg2
import sqlalchemy
# Required columns from raw table per model
raw_features = {
...
# Function to load only required columns from raw table safely
def load_raw_features(columns):
    ...
    query = text(f"SELECT {cols_str} FROM raw_customer_events")  # adjust table name
...
for model in df_filtered["model_used"].unique():
    ...
    # Load only the necessary columns from raw table
    df_raw = load_raw_features(features)
    
    if df_raw.empty:
        continue
    
    # Join with filtered clusters
