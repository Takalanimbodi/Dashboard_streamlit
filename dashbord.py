...
# Features per model (used in Cluster Profiles and Drill-Down)
raw_features = {
    "behavioral": [
        "lifetime_value",
        "membership",
...
def load_raw_features(columns):
    ...
    for attempt in range(1, 4):
        try:
            ...
            st.warning(f"Database connection failed. Retrying {attempt}/3...")
            time.sleep(2)
...
# Apply filters
df_filtered = df[
    df["model_used"].isin(models) & df["cluster_description"].isin(clusters)
].copy()
...
df_filtered = df_filtered[
    (df_filtered["scored_at"] >= start_dt) & (df_filtered["scored_at"] < end_dt)
]
...
with tab1:
    ...
    with colB:
        ...
        fig3 = px.line(trend, x="scored_at", y="count", markers=True)
        fig3.update_layout(xaxis_title="", yaxis_title="Customers Scored")
...
with tab2:
    ...
    movement_df["previous_cluster"] = movement_df.groupby("row_id")[
        "cluster_description"
    ].shift(1)
    movement_df["moved"] = (
        movement_df["previous_cluster"] != movement_df["cluster_description"]
    )

    movement_changes = movement_df[
        (movement_df["moved"]) & (movement_df["previous_cluster"].notna())
    ]

    if not movement_changes.empty:
        st.caption(f"{len(movement_changes):,} customers changed clusters")

        st.dataframe(
            movement_changes[
                ["row_id", "previous_cluster", "cluster_description", "scored_at", "model_used"]
            ],
...
with tab3:
    ...
    if not runs.empty:
        ...
        fig4.update_layout(xaxis_title="", yaxis_title="Silhouette Score")
...
# TAB 4 — CUSTOMER DRILL-DOWN

with tab4:

    st.subheader("Customer Drill-Down Analytics")

    if df_filtered["cluster_description"].empty:
        st.info("No clusters available for the current filters.")
        st.stop()
    ...
    model_for_cluster = (
        cluster_customers["model_used"].iloc[0]
    ...
    display_cols = ["row_id", "scored_at"] + [
        c
        for c in cluster_customers.columns
        ...
        for i, feat in enumerate(numeric_features):
            ...
            if col_data.empty:
                ...
                summary_cols[i].metric(
                    ...
                    delta=f"median: {col_data.median():,.1f}",
                )
    else:
    ...
    st.dataframe(cluster_customers[safe_display_cols], use_container_width=True)
